...
class StaffAccountController {
	...
	HttpEntity<?> putStaffAccountPassword(@PathVariable AccountIdentifier accountId, @Valid @RequestBody NewPassword payload, Errors errors, @LoggedIn Department department) {
		...
		// TODO CORE-231: What about the quarano admin? Do we need handle him specifically?
		if (!department.getId().equals(existing.getDepartmentId())) {
			return ResponseEntity.notFound().build();
		}
